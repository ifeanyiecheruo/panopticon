...
- [Camera2 / recording pipeline](#camera2--recording-pipeline)
  ...
  - [Calibration zoom probe](#calibration-zoom-probe)
  - [Manual Camera2 controls + multi-camera — what the Pixel 6 / BLU G5 showed](#manual-camera2-controls--multi-camera--what-the-pixel-6--blu-g5-showed)
...
### Manual Camera2 controls + multi-camera — what the Pixel 6 / BLU G5 showed
...
- **`CONTROL_ZOOM_RATIO` vs legacy `SCALER_CROP_REGION` — both live.** Pixel 6 (API 36):
  ...
  `calibration/ZoomRatioApi30` (the `NoSuchFieldError`-behind-a-guard trap — see above); the
  BLU G5 ran the capabilities read clean.
...
Out of scope for this vertical slice (no live HLS LL-HLS upgrade; calibration, motion-gated
recording and manual camera controls now exist but the specific quirks below still haven't
been re-tested against the Pixel 6) - see `panopticon-prototype/QUIRKS.md` for the original
write-ups:
...
- ~~Manual controls aren't reliably honored at all~~ — **re-verified and mostly false on the
  ...
  with large measured effects (see the "Manual Camera2 controls + multi-camera" section above).
